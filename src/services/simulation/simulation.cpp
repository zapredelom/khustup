...
khustup::models::DrawUpdate Simulation::generate_random_draw_update_model() noexcept {
    ...
    std::generate_n(std::back_inserter(coordinates), _draw_points_count_per_update, [&]() {
        ...
        return khustup::models::DrawPoint{khustup::models::Coordinate(x,y), true,khustup::models::Color{_dist_color(_gen),_dist_color(_gen),_dist_color(_gen)}};
...
khustup::models::DrawUpdate Simulation::gen_arrow() noexcept {
    ...
    for(int j = 0; j < _width; j++)
    {
        if((i>4 && i<8) && ((j>4 && j<8))||
        (i>10 && i<16) && ((j>10 && j<16)))
        coordinates.push_back(khustup::models::DrawPoint{khustup::models::Coordinate(i,j), true,khustup::models::Color{0,_dist_color(_gen),_dist_color(_gen)}});
    }
    return khustup::models::DrawUpdate(coordinates, khustup::utils::Clock::now());
}
